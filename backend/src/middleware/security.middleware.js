...
const corsOptions = {
  origin(origin, callback) {
    // Allow non-browser tools (no origin) in development OR local prod testing.
    if (!origin && (process.env.NODE_ENV !== "production" || isLocalProdTest)) {
  ...
  allowedHeaders: ["Content-Type", "Authorization", "X-Requested-With", "X-CSRF-Token", "x-refresh-token"],
  maxAge: 600,
};
  ...
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: [
        ...
        "https://*.mangalik.store",
        // Allow local dev clients when the server is run in production mode locally
        "http://localhost:*",
        "http://192.168.29.125:*",
        "ws://localhost:*",
        "ws://192.168.29.125:*",
...
const { handleRateLimitViolation } = require("./defender.middleware");
...
const apiLimiter = rateLimit({
  ...
  handler: async (req, res, _next, options) => {
    ...
    await handleRateLimitViolation(req);
...
const authLimiter = rateLimit({
  ...
  handler: async (req, res, _next, options) => {
    logger.warn(`Auth rate limit hit: ${req.ip} on ${req.originalUrl}`);
    await handleRateLimitViolation(req);
    res.status(429).json(options.message);
  },
});
